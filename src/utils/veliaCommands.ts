...
function matchNlp(text: string): string | null {
  // App list query
  if (/как(ие|ое)\s*(приложени|прилож)|список\s*приложений|поддерживаемые\s*приложения|сколько\s*приложений|какие\s*приложения\s*(ты\s*)?(поддерживаешь|умеешь|можешь|знаешь|открываешь)|какие\s*(есть\s*)?приложения|что\s*(можешь|умеешь)\s*открыть|что\s*за\s*приложени/.test(text)) {
    const appList = apps.map(a => `${a.icon} ${a.name}`).join(", ");
    return `📱 Я поддерживаю **${apps.length}** приложений!\n\n${appList}\n\nНапиши \`открой [название]\` чтобы открыть! 💜`;
  }
